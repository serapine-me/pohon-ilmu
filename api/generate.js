...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 3. AMBIL DATA BAB DARI SUPABASE ───────────────────
    ...
    // ── 4. BUILD PROMPT ────────────────────────────────────
    const diffMap = {
      mudah: 'MUDAH — C1-C2 Bloom (mengingat & memahami). Bahasa sangat sederhana, satu konsep per soal, pilihan jawaban jelas.',
      sedang: 'SEDANG — C2-C3 Bloom (memahami & menerapkan). Kontekstual, sedikit analisis, pilihan jawaban lebih mendekati.',
      sulit: 'SULIT — C3-C4 Bloom (menerapkan & menganalisis). Multi-konsep, butuh penalaran, pengecoh yang kuat.'
    };
...
2. Konsep Utama (penjelasan sesuai usia, pakai analogi jika perlu)
...
`;
    if (incl_pg) {
      prompt += `KUNCI PILIHAN GANDA:
1. [huruf jawaban]: [penjelasan singkat mengapa ini benar]
(lanjutkan untuk semua soal)
...
`;
    ...
    if (incl_essay) {
      prompt += `PANDUAN ESSAY:
1. [poin-poin jawaban yang diharapkan, 2-3 poin]
(lanjutkan untuk semua soal)
`;
    }
    ...
    if (!openaiRes.ok) {
      ...
      throw new Error(`OpenAI error: ${err.error?.message || openaiRes.status}`);
    }
